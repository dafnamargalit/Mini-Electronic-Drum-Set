...
SoftwareSerial myMP3 = SoftwareSerial(ARDUINO_RX, ARDUINO_TX);
...
//    Serial.println(inst);
    ...
    if( inst == 'k'){
        sendCommand(CMD_PLAY_W_INDEX, 0x7E04410001);//kick drum
      }
      else if( inst == 's'){
        sendCommand(CMD_PLAY_W_INDEX, 0x7E04410002);//snare
       }
      else if( inst == 'h'){
        sendCommand(CMD_PLAY_W_INDEX, 0x7E04410004);//hi hat
      }
  }
  
 
// UART Speaker Commands
